...
Использует секретные вопросы для верификации через iforgot.apple.com
...
# Load settings (for YesCaptcha API key)
settings = config.get('yescaptcha', {})
...
PROXY_PASS = proxy_config.get('pass', '')
CHANGE_IP_URL = proxy_config.get('change_ip_url', '')
CHANGE_IP_COOLDOWN = proxy_config.get('change_ip_cooldown', 65)

def get_proxy_url():
    """Get proxy URL for tls_client"""
    if PROXY_ENABLED and PROXY_HOST:
        return f"http://{PROXY_USER}:{PROXY_PASS}@{PROXY_HOST}:{PROXY_PORT}"
    return None

def change_proxy_ip():
    """Change mobile proxy IP via API"""
    ...
    def run(self) -> dict:
        ...
        try:
            ...
            for q in questions:
                ...
                answer = self.answers[answer_idx - 2]
...
def main():
    ...
    if PROXY_ENABLED:
        ...
        print(f"{Fore.CYAN}[+] IP change cooldown: {CHANGE_IP_COOLDOWN}s{Fore.RESET}\n")
    ...
    for i, account in enumerate(accounts):
        ...
        # Wait for IP change cooldown if proxy enabled and more accounts to process
        if PROXY_ENABLED and i < len(accounts) - 1:
            print(f"{Fore.CYAN}  Waiting {CHANGE_IP_COOLDOWN}s for IP change cooldown...{Fore.RESET}")
            time.sleep(CHANGE_IP_COOLDOWN)
